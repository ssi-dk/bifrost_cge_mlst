...
from bifrost_cge_mlst import launcher as bcml
...
def test_install_component(mydb):
    ...
    args: argparse.Namespace = bcml.parser(["--install"])
    bcml.run_program(args)
...
def test_pipeline(mydb):
    #TODO: this currently fails as this has a requirement of the species being set. Have to set that before this test works
    ...
    args = bcml.parser(["-id", sample_id,"-out", "/bifrost_test_data/output/test1"])
    bcml.run_program(args)
    assert os.path.isfile("/bifrost_test_data/output/test1/cge_mlst/datadump_complete")